...
from physdes.recti import Rectangle, detect_overlap


def visualize_overlap_svg(
    rectangles: list[Rectangle],
    overlapping_pair: Optional[tuple[Rectangle, Rectangle]] = None,
    width: int = 800,
    height: int = 600,
    margin: int = 50,
) -> str:
    ...
    if overlapping_pair:
        ...
        for idx, r in enumerate(rectangles):
            if (r.xcoord == r1.xcoord and r.ycoord == r1.ycoord) or (r.xcoord == r2.xcoord and r.ycoord == r2.ycoord):
    ...
    for idx, r in enumerate(rectangles):
        ...
        svg_parts.append(f'<rect x="{x1}" y="{y1}" width="{w}" height="{h}" fill="{fill}" stroke="{stroke}" stroke-width="2" opacity="{opacity}"/>')
...
def demo() -> None:
    ...
    overlap_result = detect_overlap(svg_rects)
    ...
    svg = visualize_overlap_svg(svg_rects, overlap_result)
