"""
Rectangle, VSegment, and HSegment classes for axis-aligned 2D geometry.

``Rectangle`` is a ``Point`` of two ``Interval`` instances. ``VSegment`` and
``HSegment`` represent vertical and horizontal line segments respectively.
Includes a sweep-line overlap detector via ``detect_overlap_gen``.
"""
