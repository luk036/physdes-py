...
class ClockTree3dVisualizer:
    ...
    def _draw_wires3d(self, root: Any, scale_coord: Callable[[Any, Any], Tuple[float, float]]) -> List[str]:
        ...
        def draw_wires_recursive(node: Any) -> None:
            ...
            if node.parent:
                ...
                x_start, y_start = scale_coord(node.parent.position.xcoord.xcoord, node.parent.position.ycoord)
                x_end, y_end = scale_coord(node.position.xcoord.xcoord, node.position.ycoord)

                svg_elements.append(f'<line x1="{x_start}" y1="{y_start}" x2="{x_end}" y2="{y_end}" stroke="{self.wire_color}" stroke-width="{self.wire_width}" stroke-linecap="round"/>')

                # Add wire length label
                mid_x = (x_start + x_end) / 2
                mid_y = (y_start + y_end) / 2
                if hasattr(node, "wire_length") and node.wire_length > 0:
                    svg_elements.append(f'<text x="{mid_x}" y="{mid_y - 5}" class="wire-label" text-anchor="middle">{node.wire_length:.1f}</text>')
    ...
    def _draw_nodes(
        ...
        svg_elements = []
        sink_positions = {(sink.position.xcoord.xcoord, sink.position.ycoord) for sink in sinks}

        def draw_nodes_recursive(node: Any, depth: int = 0) -> None:
            if not node:
                return

            x_pos, y_pos = scale_coord(node.position.xcoord.xcoord, node.position.ycoord)
            ...
            # Draw node circle
            svg_elements.append(f'<circle cx="{x_pos}" cy="{y_pos}" r="{radius}" fill="{color}" stroke="#333" stroke-width="1"/>')

            # Draw node label
            label_y_offset = -radius - 5
            svg_elements.append(f'<text x="{x_pos}" y="{y_pos + label_y_offset}" class="node-label" text-anchor="middle">{node.name}</text>')

            # Draw delay information if available
            if hasattr(node, "delay"):
                delay_y_offset = radius + 12
                svg_elements.append(f'<text x="{x_pos}" y="{y_pos + delay_y_offset}" class="delay-label" text-anchor="middle">d:{node.delay:.1f}</text>')

            # Draw capacitance information for sinks
            if is_sink and hasattr(node, "capacitance"):
                cap_y_offset = radius + 22
                svg_elements.append(f'<text x="{x_pos}" y="{y_pos + cap_y_offset}" class="delay-label" text-anchor="middle">c:{node.capacitance:.1f}</text>')
...
def create_interactive_svg(
    ...
    svg_content = visualizer.visualize_tree3d(root, sinks, filename, width, height, analysis)
    return svg_content
    ...
    for i, tree_data in enumerate(trees_data):
        ...
        for line_idx, line in enumerate(lines):
            if '<g class="clock-tree3d">' in line:
                start_idx = line_idx
            elif start_idx != -1 and "</g>" in line and "clock-tree3d" not in lines[line_idx - 1]:
                end_idx = line_idx
                break
