"""
Generic polymorphic operations for VLSI physical design.

Functions (overlap, contain, intersection, min_dist, nearest, measure_of,
center, lower, upper, displacement) dispatch on ``hasattr`` to work with
scalars, Interval objects, or any type exposing the appropriate protocol
method (e.g. ``overlaps``, ``contains``, ``intersect_with``).
"""
...
def overlap(lhs: Any, rhs: Any) -> bool:
    ...
    :param rhs: The parameter `rhs` is the right-hand side of the comparison. It can be any object
        that supports the `overlaps` method or a scalar value
...
def intersection(lhs: Any, rhs: Any) -> Any:
    ...
    :param lhs: The `lhs` parameter represents the left-hand side of the intersection operation,
        while the `rhs` parameter represents the right-hand side of the intersection operation
...
def displacement(lhs: Any, rhs: Any) -> Any:
    ...
    :param lhs: The `lhs` parameter represents the left-hand side of the displacement operation. It
        can be either an object that has a `displace` method or a scalar value

    :param rhs: The `rhs` parameter represents the displacement value that needs to be subtracted
        from the `lhs` parameter

    :return: the displacement between `lhs` and `rhs`. If `lhs` has a `displace` method, it calls
        that method passing `rhs` as an argument. Otherwise, it assumes `lhs` is a scalar and
