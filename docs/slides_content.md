...
**1. The Problem** 🤔
- Why Figures Are Essential
- The Asset Pipeline Gap
- Real-World Impact

**2. Python Solutions** 🐍
- `.. figure::` — static images
- `.. plot::` — auto-generated plots
...
| Feature | Status |
|---------|--------|
| Static images | ❌ No native asset bundling |
| Auto-generated plots | ❌ Not available |
| ASCII→SVG diagrams   | ✅ `svgbob` crate     |
| Equations | ✅ `$$...$$` inline |
| Asset pipeline | ⚠️ `embed-doc-image` (workaround) |
...
**Rust has no macro or tool** to run code and capture output during `cargo doc`.
]
...
✅ `sphinxcontrib-svgbob` extension
...
✅ `svgbobdoc` crate (proc macro)
✅ ` ```svgbob ` in doc comments
✅ SVG embedded via `transform!`
✅ docs.rs compatible

**Rust uses `svgbobdoc`** — a proc-macro crate that converts ASCII diagrams to SVG at compile time.
]

---

### 🦀 Rust `svgbobdoc` — Setup
...
**3. Feature-gating** (optional, to skip macro at compile time):

```toml
[features]
default = ["doc-svg"]
doc-svg = ["svgbobdoc/enable"]
```

**1.6M+ downloads** · 16 reverse dependents · Requires Rust 1.54+
...
**Python**: Native tools — files copied, code executed, ASCII converted. All built into Sphinx.
**Rust**: Same features via crates — `embed-doc-image` for images, `svgbob` for diagrams.
...
4. **RST verbosity**
   - Directives more verbose than Markdown
]
...
| Need | Solution | Lines |
|------|----------|-------|
| Static image | `.. figure::` | 0 |
| Auto plot | `.. plot::` | +1 ext |
| ASCII diagram | `.. svgbob::` | +1 ext |
| Equation | `.. math::` | 0 |
| Asset pipeline | Auto-copy | 0 |
...
| Need | Solution | Lines |
|------|----------|-------|
| Static image | `embed-doc-image` | 6+20 |
| Auto plot | ❌ Not available | Pre-render externally |
| ASCII diagram | `svgbob` crate | 0 (built-in) |
| Equation | `$$...$$` | 0 |
| Asset pipeline | `embed-doc-image` | Same |

**Rust's `svgbob` gives parity for diagrams. Image bundling still needs a crate.**
]
...
**Slides**: [`luk036.github.io/idea/doc-fig-py-remark-v2`](https://luk036.github.io/idea/doc-fig-py-remark-v2.html)
