...
triangle_strategy = st.builds(
    ...
    pointset=st.lists(point_strategy, min_size=3, max_size=3)
)
...
convex_quad_strategy = st.builds(
    ...
    pointset=st.lists(
        ...
        min_size=4, 
        max_size=4
    ).filter(lambda points: len(set((p.xcoord, p.ycoord) for p in points)) == 4)
)
...
class TestPolygonEdgeCases:
    ...
    @given(numeric_values, numeric_values, numeric_values, numeric_values, numeric_values, numeric_values)
